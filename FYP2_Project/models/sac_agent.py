...
class MixedReplayBuffer:
    ...
    def load_expert_data(self, expert_data_root):
        ...
        self.expert_v = torch.empty((total_steps, 12, IMG_DIM_Y, IMG_DIM_X*2), dtype=torch.uint8, device=self.device)
        ...
        self.expert_nv = torch.empty((total_steps, 12, IMG_DIM_Y, IMG_DIM_X*2), dtype=torch.uint8, device=self.device)
        ...
        for episode_data in fill_pbar:
            for t in episode_data:
                ...
                v_processed = v_raw.permute(0, 3, 1, 2).reshape(12, IMG_DIM_Y, IMG_DIM_X*2)
                ...
                nv_processed = nv_raw.permute(0, 3, 1, 2).reshape(12, IMG_DIM_Y, IMG_DIM_X*2)
